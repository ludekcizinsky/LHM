...
class MultiHumanFinetuner(Inferrer):
    ...
    def _load_gs_model(self, root_output_dir: Path):
        ...
        for track_id in track_ids:
            ...
            self.gs_model_list.extend(
                torch.load(gs_model_dir / "gs_model_list.pt", map_location=self.tuner_device)
            )
            self.query_points.append(torch.load(gs_model_dir / "query_points.pt", map_location=self.tuner_device))
            self.transform_mat_neutral_pose.append(torch.load(
                gs_model_dir / "transform_mat_neutral_pose.pt", map_location=self.tuner_device
            ))

        self.transform_mat_neutral_pose = torch.cat(self.transform_mat_neutral_pose, dim=0)  # [P,4,4]
        self.query_points = torch.cat(self.query_points, dim=0)  # [P,N,3] 

        print(f"Loaded GS models for {len(self.gs_model_list)} human tracks.")
        print(f"Shape of query points: {self.query_points.shape} and transform mat neutral pose: {self.transform_mat_neutral_pose.shape}.")
